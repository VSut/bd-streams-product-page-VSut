...
//        return Optional.empty();
        return productV2.buyingOptions().stream().findFirst();
    }
    ...
    public Optional<String> extractMainImageUrl(Integer longestDimension) {
        return productV2.productImages().map(ProductImagesV2::images).orElse(Collections.emptyList()).stream().map(image -> extractImageUrl(image, longestDimension)).filter(Objects::nonNull).findFirst();
    }
